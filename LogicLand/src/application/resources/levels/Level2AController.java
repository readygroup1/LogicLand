...
public class Level2AController extends sandboxController implements Initializable {
	...
	// ----------------Variables ---------------------------------------
		...
		// --------------------Initializer-----------------------------------------
		/**
		 * Initializes the Level2AController.
		...
		public void CheckWin() {
			...
			if(endBulb.getState()) {
		        ...
		        alert.setContentText("Great Job! Level 2A Completed!");

		        // This will block the user input until the modal dialog is dismissed
		        alert.showAndWait();
				if(AccountManager.getLevelScore(AccountManager.getLevelID(2)) != 50) {
					AccountManager.setLevelScore(AccountManager.getLevelID(2), 50);
				}
			} else {
