...
class notControllerTest extends platformTest{

	static Pane pane;
	static Pane gate;
	static notController controller;
		...
		if(!platformStarted) {
			...
			Platform.startup(() -> {
				...
				try {
					...
					loader = new FXMLLoader(notControllerTest.class.getResource("/application/resources/gates/not.fxml"));
					gate = loader.load();
	...
	void typeTest() {
		...
		Platform.runLater(() -> {
			...
			if(controller.getType() == gateObject.Type.not) {
				testStatus.set(true);
			}
			latch.countDown();
		});
