...
class nandControllerTest extends platformTest {

	static Pane pane;
	static Pane gate;
	static nandController controller;
	...
	static void setUpBeforeClass() throws Exception {
			...
			if(!platformStarted) {
				...
				try {
					...
					loader = new FXMLLoader(nandControllerTest.class.getResource("/application/resources/gates/nand.fxml"));
					gate = loader.load();
	...
	void typeTest() {
		...
		Platform.runLater(() -> {
			...
			if(controller.getType() == gateObject.Type.nand) {
				testStatus.set(true);
			}
			latch.countDown();
		});
